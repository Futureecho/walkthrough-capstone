...
def _send(to: str, subject: str, html: str) -> bool:
    ...
    try:
        resend.Emails.send({
            "from": "PropertyInspect <noreply@propertyinspect.app>",
            "to": [to],
            "subject": subject,
            "html": html,
        })
...
def send_invite_email(email: str, token: str, company_name: str, invited_by: str) -> bool:
    ...
    <p>{invited_by} has invited you to join <strong>{company_name}</strong> on PropertyInspect.</p>
    <p><a href="{url}" style="display:inline-block;padding:12px 24px;background:#2563eb;color:#fff;text-decoration:none;border-radius:6px;">Accept Invite</a></p>
    <p>This invite expires in 7 days.</p>
    <p style="color:#888;font-size:12px;">If you didn't expect this, you can ignore this email.</p>
    """
    return _send(email, f"Join {company_name} on PropertyInspect", html)
...
def send_password_reset_email(email: str, token: str) -> bool:
    ...
    <p>Click the button below to reset your PropertyInspect password.</p>
    ...
    """
    return _send(email, "Reset your PropertyInspect password", html)
